 ...
 */
final class Version20180921165419 extends AbstractMigration
{
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('ALTER TABLE caller ADD COLUMN blocked BOOLEAN NOT NULL default false ');
    }
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('DROP TABLE __temp__call');
        $this->addSql('CREATE INDEX IDX_CC8E2F3EA5626C52 ON call (caller_id)');
        $this->addSql('CREATE TEMPORARY TABLE __temp__caller AS SELECT id, number, note, added FROM caller');
        $this->addSql('DROP TABLE caller');
        $this->addSql('CREATE TABLE caller (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, number VARCHAR(15) NOT NULL, note VARCHAR(255) DEFAULT NULL, added DATETIME NOT NULL)');
        $this->addSql('INSERT INTO caller (id, number, note, added) SELECT id, number, note, added FROM __temp__caller');
        $this->addSql('DROP TABLE __temp__caller');
    }
}
